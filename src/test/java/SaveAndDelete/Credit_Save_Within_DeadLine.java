...
/* #######  Test for accommodation saving itenary for Credit user within Deadline #########
######  Scenario Logs In, Searches a specified hotel and then saves the itenary   ##### */

public class Credit_Save_Within_DeadLine {
	...
	Logger logger = Logger.getLogger("Credit_Save_Within_DeadLine");

	/* ####### Passing browser as parameters in test ######### **/

	@Test
	@Parameters({ "browsername" })
	public void CreditSaveWithinDeadLine(String browsername) throws Exception {
		...
		try {
			...
			System.out.println(expectedaftrsavehotel);
			String actualaftrsavehotel = driverqa.findElement(Booking.AfterSaveHotel).getText();
			System.out.println(actualaftrsavehotel);
			...
			// driverqa.findElements(Booking.ItnCart).size());
