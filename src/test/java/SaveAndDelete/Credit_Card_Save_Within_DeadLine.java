...
/* #######  Test for accommodation saving itenary for Credit Card user within Deadline #########
######  Scenario Logs In, Searches a specified hotel and then saves the itenary   ##### */

public class Credit_Card_Save_Within_DeadLine {
	...
	Logger logger = Logger.getLogger("Credit_Card_Save_Within_DeadLine");

	/* ####### Passing browser as parameters in test ######### **/

	@Test
	@Parameters({ "browsername" })
	public void CreditCardSaveWithinDeadLine(String browsername) throws Exception {
		...
		try {
			...
			obj.Takesnap(driverqa,
					Config.SnapShotPath() + "/Save/Error/Accommodation_Save_Credit_Card/Search-Result.jpg");
		...
		try {
			...
			Thread.sleep(2000);
			obj.Takesnap(driverqa,
					Config.SnapShotPath() + "/Save/Accommodation_Save_Credit_Card/Passenger-Details.jpg");
